...
/-!
# The `CircOp` circuit DSL on `Free`
...
`Code` AST, sound against `ofFree` by `≈`.
-/
...
/-- `.2`: the AST's ITree meaning `denoteProg` is `≈ ofFree` of the source — note `denoteProg` lands
    in `Comp` directly; `ofFree` only embeds the source `Free` for comparison. -/
...
Soundness now goes through a **compositional bisimulation** (no `Free` bridge): the reflector
unfolds the source definitions it touched so scope- and call-binds fuse consistently on both sides,
and `denoteProg ≈ ofFree` closes. -/
...
as an `HList`.  Here `checkSquare` takes `x : Nat`, and soundness is `∀ x, … ≈ ofFree (checkSquare x)`. -/
